--[[ In-game Derma — sends changes to server (superadmin) ]]
...
function SND.OpenSettingsMenu()
	...
	f:SetTitle("Search & Destroy — settings (server)")
	f:SetSize(420, 520)
	f:Center()
	f:MakePopup()
	...
	local defs = {
		{ key = "snd_walk_speed", lbl = "Walk speed", min = 120, max = 320 },
		{ key = "snd_run_speed", lbl = "Run speed", min = 160, max = 400 },
		{ key = "snd_sprint_mult", lbl = "Sprint multiplier", min = 1, max = 2.2, dec = 2 },
		{ key = "snd_round_time", lbl = "Round time (sec)", min = 60, max = 300 },
		{ key = "snd_freeze_time", lbl = "Freeze time (sec)", min = 0, max = 15 },
		{ key = "snd_plant_time", lbl = "Plant time (sec)", min = 2, max = 10, dec = 1 },
		{ key = "snd_defuse_time", lbl = "Defuse time (sec)", min = 3, max = 12, dec = 1 },
		{ key = "snd_win_limit", lbl = "Rounds to win", min = 1, max = 16 },
		{ key = "snd_bot_count", lbl = "Bots", min = 0, max = 16 },
		{ key = "snd_bot_skill", lbl = "Bot skill", min = 0.1, max = 1, dec = 2 },
		{ key = "snd_team_balance", lbl = "Team balance (0/1)", min = 0, max = 1 },
		{ key = "snd_mapvote_enabled", lbl = "Map vote (0/1)", min = 0, max = 1 },
	...
	for _, row in ipairs(defs) do
		...
		if row.dec then sl:SetDecimals(row.dec) end
		sl:SetValue(cur)
