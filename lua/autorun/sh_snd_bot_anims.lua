...
hook.Add("UpdateAnimation", "SND_SharedAnimLogic", function(ply, vel, maxSeqGroundSpeed)
	...
	-- Enable procedural logic for bots OR any player using a CSS rig (legacy system)
	if not ply.SND_IsBot and not ply:IsBot() and not isLegacyRig(ply) then return end

	if not ply._snd_anim_logic_started then
		ply._snd_anim_logic_started = true
		MsgC(Color(255, 120, 0), "[SND_ANIM] ", Color(0, 255, 0), "Procedural Pose System engaged for " .. ply:Nick() .. "\n")
	end
