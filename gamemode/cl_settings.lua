...
function SND.OpenSettingsMenu()
	...
	local categories = {
		["Match Rules"] = {
			...
			{ key = "snd_bomb_fuse_time",   lbl = "Bomb Fuse Time (sec)", min = 15,   max = 90,   dec = 0 },
			{ key = "snd_rust_swap_spawns", lbl = "Rust: Swap Spawns",    min = 0,    max = 1,    dec = 0 },
		},
		["Combat & Movement"] = {
			...
			{ key = "snd_ads_slow",         lbl = "ADS Speed Multiplier", min = 0.5,  max = 1.0,  dec = 2 },
			{ key = "snd_air_accel_scale",  lbl = "Air Acceleration",     min = 0,    max = 5,    dec = 2 },
			{ key = "snd_plant_time",       lbl = "Bomb Plant Time",      min = 2,    max = 10,   dec = 1 },
			{ key = "snd_defuse_time",      lbl = "Bomb Defuse Time",     min = 3,    max = 12,   dec = 1 },
			{ key = "snd_stamina_drain",    lbl = "Stamina Drain Rate",   min = 0.05, max = 1.0,  dec = 2 },
			{ key = "snd_stamina_recover",  lbl = "Stamina Recovery",     min = 0.05, max = 1.0,  dec = 2 },
			{ key = "snd_stamina_jump_cost",lbl = "Jump Stamina Cost",    min = 0,    max = 0.5,  dec = 2 },
			{ key = "snd_health_regen_delay",lbl = "Health Regen Delay",  min = 1,    max = 10,   dec = 1 },
			{ key = "snd_health_regen_rate", lbl = "Health Regen Rate",   min = 1,    max = 20,   dec = 0 },
		},
		["Bots & AI"] = {
...
function SND.OpenPersonalizationMenu()
	...
	local function drawFullPreview(pnl, w, h, av)
		...
		if ePath == "steam" then
			...
			if eMat and not (eMat:IsError() and not tostring(ePath):match("[.gif|data/]")) then
		...
		if showTitleCheck and showTitleCheck:GetChecked() and not (useTitleMatCheck and useTitleMatCheck:GetChecked()) then
			...
			draw.SimpleText(tTxt:upper(), "SND_BO3_Team", textX + 1, 36 * sc_local, Color(0, 0, 0), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
			draw.SimpleText(tTxt:upper(), "SND_BO3_Team", textX, 35 * sc_local, Color(255, 210, 50), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		end

		local tCol = team.GetColor(lp:Team())
		draw.SimpleText(lp:Nick():upper(), "SND_BO3_Player", textX + 1, 93 * sc_local, Color(0, 0, 0), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		draw.SimpleText(lp:Nick():upper(), "SND_BO3_Player", textX, 92 * sc_local, Color(tCol.r, tCol.g, tCol.b), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		...
		if icon then
			...
			surface.SetDrawColor(255, 255, 255)
			surface.DrawTexturedRect(w - 55 * sc_local, cardH * 0.5 - 20 * sc_local, 40 * sc_local, 40 * sc_local)
		end
